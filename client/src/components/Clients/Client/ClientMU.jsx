        ...
        <Grid  style={{ minHeight: '700px' }}>
          <Paper style={{ minHeight: '600px', overflowY: 'scroll' }} >
            ...
            {client?.comments?.length
                ? client.comments.map(comment => (
                  <TableRow key={comment._id} >
                    <TableCell>{`${comment.manager?.lastName} ${comment.manager?.name[0]}. ${comment.manager?.middleName[0]}.`} :</TableCell>
            ...
            {/* <ul>
              {client?.comments?.length
                ? client.comments.map(comment => (
                  <li key={comment._id}>{`${comment.manager?.lastName} ${comment.manager?.name[0]}. ${comment.manager?.middleName[0]}.`} : {comment.text}{new Date(comment.createdAt).toLocaleString()}</li>
                ))
                : null
              }
            </ul> */}
